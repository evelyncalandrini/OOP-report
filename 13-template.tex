...
\subsection*{Evelyn Calandrini - Gestione del Tavolo, Valutazione dei Round, Statistiche e View}
...
\textbf{Gestone dello Stato e delle puntate}
\\
\begin{figure}[H]
	\centering{}
	...
	\label{img:design}
\end{figure}
\textbf{Problema} : In un ambiene di gioco a turni, è neccessario garantire che le azione dei singoli giocatori (come effettuare una puntata o chiedere le carte) avvengano esclusivamente nelle fasi corrette di gioco, prevendendo stati inconsistenti o alterazioni 
illecite del piatto.
\\ 
\textbf{Soluzione} : Il sistema utilizza un approccio basato su enumerazione (State) per indentificare le varie fasi di gioco e con il relativo metodo di transazione (next())
Il tavolo (TableImpl) verfica lo stato corrente prima di ogni operaizone. In questo modo le varie azione di gioco controllano prima di essere nello stato corretto, se l'azione non è ammessa l'esecuzione viene interrotta lanciando un eccezione. 
