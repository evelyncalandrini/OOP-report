...
\subsection*{Beatrice Pallotti- Gestione dei round speciali, gestione dei turni e del flusso di gioco}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{UML_specialRound}
	\caption{diagramma relle classi special round}
\end{figure}

\subsection*{Creazione e gestione dei round speciali}
\paragraph*{problema:} Il gioco ChaosJack prevede la presenta di più round speciali, ciascuno caratterizzato da delle precise regole per il calcolo dei punteggi delle mani dei giocatori. 
...
\paragraph*{Soluzione:} Per evitare il problema spiegato sopra si è optato per l'utilizzo del pattern di tipo Strategy che mi permette di andare a gestire in maniera dinamica l'attivazione oppure no dei round speciali al'interno del gioco.
\begin{enumerate}
	\item L'interfaccia \texttt{SpecialRound} è la \texttt{Strategy Interface} contenente il metodo \texttt{specialSCore(List<Card> playerscards)}. 
	\item Le \texttt{Concrete Strategies} sono le tre classi che implementano in maniera concreta i round speciali, esse sono: \texttt{DoubleheartsRule,YingYung e RoyalFreezeTurn}. Ognuna di essa implementa le proprie regole per il calcolo del punteggio.
	\item Il \texttt{Context} è la classe \texttt{GameEngineImpl} la quale ha \texttt{Optional<SpecialRound>} come riferimento al round attivo in quel momento del gioco ( null se il round attivo utilizza le regole base del gioco).Tramite il metodo \texttt{currentScore()} viene gestito il calcolo del punteggio richiamando le regole del round attivo.
\end{enumerate}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{UML_model-controller.png}
	\caption{diagramma relativo al Gameengine e il GameFlowController}
\end{figure}

\subsection*{Model e controller}
\paragraph*{Problema:} per determinate azioni del gioco è richiesta la coordinazione e il richiamo di più classi del dominio, un esempio può essere la gestione dei turni dei giocatori, la logica che permetta al giocatore di poter passare il proprio turno in maniera volontaria e , come detto prima, la logica con cui selezionare e implementare la regola per il calcolo dei punteggi.
Se dovessimo delegare il controller alla gestione di tutte queste logiche il codice diventerebbe troppo pesante dato che si dovrebbe già occupare della gestione del flusso di gioco.

\paragraph*{Soluzione:} Per risolvere il problema è stata creata la classe \texttt{GameEngineImpl} al cui interno si trovano i metodi che servono a gestire le logiche più complesse trattate sopra come ad esempio \texttt{nextTurn()} (per i turni dei giocatori), \texttt{stand} (per permettere ai giocatori di passare il turno volontariamente), \texttt{currentScore} (per applicare le giuste regole di calcolo del gioco), \texttt{resetGame} (che permette di andare a inizializzare il gioco ogni volta che si inizia un round).
Con questa soluzione il controller potrà utilizzare queste logiche all'interno del suo codice andando a richiamare dei semplici metodi.
...
\subsection*{Beatrice Pallotti}
\begin{itemize}
	\item \textbf{Utilizzo della classe Optional}
	Utilizzata per il riferimento al round speciale.
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/model/impl/GameEngineImpl.java#L28}
	\item \textbf{Utilizzo di stream}
	Utilizzato per controllare  che tutti i giocatori (ad eccezione del dealer) abbiano sballato ( abbiano uno score maggiore di 21).
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/controller/impl/GameFlowControllerImpl.java#L236}
	\item \textbf{Utilizzo delle lambda}
	Utilizzate nel collegamento con i bottoni della view
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/controller/impl/GameFlowControllerImpl.java#L114}
	\item \textbf{Utilizzo libreria esterna JavaFX}
	Ho utilizzato questa libreria per implementare il PauseTransition .Esso è stato utilizzato per far si che ci sia una pausa temporale tra le azioni dei bot (npc e dealer)
	Utilizzo questa libreria per utilizzare Platform.exit() per chiudere l'interfaccia grafica.
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/controller/impl/GameFlowControllerImpl.java#L302}
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/controller/impl/GameFlowControllerImpl.java#L86}
	\item \textbf{Utilizzo annotazioni di librerie di terze parti (SpotBugs)}
	\texttt{@SuppressFBWarnings} è stata utilizzata per gestione dell'esposizione intenzionale a riferimenti esterni
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/controller/impl/GameFlowControllerImpl.java#L53}
	Permalink:\url{https://github.com/evelyncalandrini/OOP25-chaosjack/blob/a372dd05d690b559e4186b1993c1903b468c322d/src/main/java/it/unibo/chaosjack/model/impl/GameEngineImpl.java#L115}
\end{itemize}

	l'esempio di utilizzo per il pauseTransition è stato preso dal seguente link: \url{https://stackoverflow.com/questions/27334455/how-to-close-a-stage-after-a-certain-amount-of-time-javafx}

    \subsection*{Elena Lungu}                                                                           
    \begin{itemize}                                                                                     
        \item \textbf{Utilizzo della classe Optional}                                                   
        Utilizzata all'interno della classe \texttt{StandardDeck} per gestire in modo sicuro la pesca di
  una carta dal mazzo, prevenendo la restituzione di valori nulli qualora il mazzo fosse vuoto.         
        Permalink: \url{https://github.com/evelyncalandrini/OOP25-                                      
  chaosjack/blob/master/src/main/java/it/unibo/chaosjack/model/impl/StandardDeck.java#L47-L51}          
                                                              
        \item \textbf{Utilizzo della libreria JavaFX e costrutti SVGPath}                               
        Oltre all'impiego per la struttura generale delle viste del portafoglio e delle singole carte   
  (\texttt{CardViewImpl} e \texttt{PlayerWalletViewImpl}), la libreria è stata sfruttata a un livello   
  più avanzato tramite \texttt{javafx.scene.shape.SVGPath} per disegnare programmaticamente in formato  
  vettoriale i simboli dei modificatori sulle carte.                                                    
        Permalink: \url{https://github.com/evelyncalandrini/OOP25-                                      
  chaosjack/blob/master/src/main/java/it/unibo/chaosjack/view/impl/CardViewImpl.java#L164-L188}         
        
        \item \textbf{Utilizzo della classe di utilità Collections}
        Impiegata in combinazione con la classe \texttt{StandardDeck} per applicare algoritmi di        
  randomizzazione standard (\texttt{Collections.shuffle}) sia al mazzo di carte principale che al pool  
  di modificatori speciali, garantendo un'adeguata casualità distributiva.
        Permalink: \url{https://github.com/evelyncalandrini/OOP25-
  chaosjack/blob/master/src/main/java/it/unibo/chaosjack/model/impl/StandardDeck.java#L60-L61}          
        
        \item \textbf{Utilizzo estensivo di Enum per il Domain Driven Design}
        Sfruttamento avanzato delle enumerazioni (es. \texttt{Suit}, \texttt{Rank},
  \texttt{CardModifier}) per modellare in modo robusto le caratteristiche immutabili delle carte da     
  gioco senza l'ausilio di magic strings o magic numbers.
        Permalink: \url{https://github.com/evelyncalandrini/OOP25-
  chaosjack/blob/master/src/main/java/it/unibo/chaosjack/model/impl/StandardCard.java}
    \end{itemize}
...
\subsection*{Beatrice Pallotti}
il mio contrinuto per il progetto è stato nella gestione dei round speciali, nella gestione dei turni, nel creare la mano del giocatore e il controllo di flusso del gioco.
Le parti più impegnative sono state su come controllare il flusso dovendo tener conto delle regole stabilite per il gioco e il coordinamento di tutte le classi del domnio dovendo utilizzarle tutte insieme per implementare determinate logiche.
Questa esperienza è stata molto formativa dal punto di vista di come approcciarsi a un progetto e su come doversi rapportare con il proprio team cercando sempre di spiegarsi il più chiaramente possibile.

\subsection*{Elena Lungu}
Il mio contributo al progetto si è focalizzato sulla        
  modellazione e implementazione del mazzo di carte (Deck e   
  relative carte) e del portafoglio dei giocatori (Wallet) a  
  livello di Model, oltre che sulla loro rappresentazione     
  visiva all'interno della View. La parte più impegnativa è   
  stata la gestione della logica delle carte e sopratutto delle carte speciali, e il corretto funzionamento del Wallet. Dal lato della View, la sfida         
  principale è stata creare componenti grafiche (CardView e   
  PlayerWalletView) che si aggiornassero dinamicamente e in   
  modo pulito in risposta ai cambiamenti del Model. Ritengo il
  lavoro complessivo molto soddisfacente, con una netta       
  separazione delle responsabilità che rispetta pienamente il 
  pattern MVC. Questa esperienza è stata particolarmente      
  formativa perché mi ha permesso di capire sul campo quanto  
  sia fondamentale progettare interfacce (API) chiare per     
  permettere agli altri membri del team di usare il mio codice
  senza doverne conoscere l'implementazione interna. In       
  un'ottica di sviluppo futuro, il sistema del Wallet potrebbe
  essere ampliato introducendo meccaniche di prestiti o       
  ricompense giornaliere, mentre l'interfaccia grafica delle  
  carte trarrebbe enorme beneficio dall'aggiunta di animazioni
  fluide per la distribuzione e lo scarto, rendendo           
  l'esperienza di gioco molto più immersiva.  
