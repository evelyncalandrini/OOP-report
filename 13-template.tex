...
Il software ChaosJack, è un viedeogioco che va a fondere il gioco classico di Black Jack con le meccaniche strategiche e i modificatori tipici di giochi come Balatro, un incontro tra il Poker classico 
e un videogioco di strategia e "rompicapo". L'obiettivo principale del giocatore è quello di sfidare e vincere contro il banco. Il banco da due carte al giocatore, e queste sono le sue carte di partenza,
...
Se la somma delle carte in mano supera 21 il giocatore è fuori e vince automaticamente il banco. Quando il giocatore decide di fermarsi il turno passa al banco e pesca lui stesso dal mazzo le carte scartando sempre la prima e girando la seconda sommando di volta in volta 
il valore delle carte girate. Il banco ha l'obbligo di arrivare almeno a 17 di punteggio, poi può decidere se continuare a pescare o a fermarsi. Il superamento del valore 21 vale anche per il banco, in caso succeda il banco è fuori e vince automaticamente il giocatore.
A differenza del Blackjack tradizionale, nel mazzo saranno presenti 4 jolly (2 rossi e 2 neri), che daranno un vantaggio a seconda di chi li pesca. Inoltre c'è la possibilità di triplicare la vincita se tutte le carte in mano, con cui si vince la partita, siano dello stesso colore.
Tuttavia non ci sono solo vantaggi, ma anche delle carte che possono svantaggiare chi le pesca, queste carte sono dette "maleddette". Una carta viene etichettata come maledetta quando il giocatore che la pesca supera 21. Iniziata la nuova partita, quando il giocatore pesca una carta "maledetta" vedrà il valore di questa raddoppiato.
Ogni due partite giocate sarà presente un round speciale, dove verranno posti degli ulteriori limiti durante il gioco (ad esempio, tutte le carte di fiori verranno pescate a faccia in giù oppure in questo round puoi pescare solo una carta). 
...
\begin{itemize}
	\item L'interfaccia utente deve sincronizzarsi in modo fluido e reattivo con la logica di gioco, aggiornandosi istantaneamente a ogni variazione di stato.
	\item Il sistema deve essere progettato in modo estensibile, garantendo la possibilità di aggiungere nuovi turni speciali o regole senza dover alterare le meccaniche di base del gioco.
\end{itemize}
