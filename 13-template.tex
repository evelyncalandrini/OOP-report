...
e un videogioco di strategia e "rompicapo". L'obiettivo principale del giocatore è quello di sfidare e vincere sia contro il banco sia contro gli altri giocatori presenti al tavolo. Oltre ai giocatori effettivi saranno presenti dei giocatori bot, che seguiranno le regole di gioco in automatico.Il giocatore per partecipare alla partita deve fare una puntata base. Successivamente il banco da due carte ad ogni giocatore, e queste sono le sue carte di partenza, che verranno date scoperte.
Successivamente il banco scarta una carta e gira la successiva a faccia scoperta sul tavolo. Il valore della carta è 10 se è una figura, può valere 1 o 11 se è un asso a descrizione di chi la pesca, 
il restante delle carte valgono quanto indicato nella carta stessa. A questo punto il giocatore in base al punteggio delle sue carte decide se chiedere un ulteriore carta al banco, la richiesta può essere effettuta più volte durante il suo turno.
Se durante la richieste delle carte, la somma di esse in mano supera 21, il giocatore in turno perde automaticamente ed è fuori dalla partita. Il turno di un giocatore finisce quando decide di "stare", ovvere quando non vuole pescare ulteriori carte.
Poi il turno passa al giocatore successivo. Il banco quando ha finito di servire tutti i giocatori presenti al tavolo, questi possono fare delle ulteriori scommesse, scegliendo una somma a sue discrizione o raddoppiando quanto messo precedentemente, prima che giochi il banco.
Finita la fase finale di scomesssa dei giocatori, il turno passa al banco e pesca lui stesso dal mazzo le carte, scartando sempre la prima e girando la seconda sommando di volta in volta 
il valore delle carte girate. Il banco ha l'obbligo di arrivare almeno a 17 di punteggio, poi può decidere se continuare a pescare o a fermarsi. 
Il superamento del valore 21 vale anche per il banco, in caso succeda il banco è fuori e la vincita verrà contesa tra chi ha il punteggio più alto tra i giocatori ancora attivi. In questo situazione se i giocatori pareggiano con stesso punteggio e stesso numero di carte, la vincita verrà raddoppiata e divisa tra i giocatori.
Se il banco non è fuori si hanno diverse caratteristiche per decretare il vincitore, essendo un tutti contro tutti, per vincere bisogna battere sia il banco che gli altri giocatori. In caso di pareggio con il banco o un altro giocatore vince chi ha meno carte in mano.
Alla fine della partita se vince solo un giocatore, egli otterrà come vincita la somma del piatto raddoppiata. Nel piatto sono presenti le fiches scommesse da tutti giocatori durante la partita. Se la vincita viene fatta con il punteggio massimo, ovvero si fa Blackjack, il valore del piatto viene triplicato, se invece viene fatta con una mano avente carte tutte dello stesso colore, si avrà un bonus 
dove la vincita viene triplica. In fine in caso di vincita con Blackjack e bonus, il valore del piatto viene moltiplicato per cinque.
All'avvio di una partita il giocatore potrebbe imbattersi in una partita "speciale", dove sono inseriti dei vincoli che modificano i punteggi delle carte.
Inoltre a differenza del Blackjack tradizionale, nel mazzo saranno presenti delle carte speciali con un proprio punteggio.
...
\begin{itemize}
	...
	\item Il gioco deve incrementare automaticamente la difficoltà nei turni speciali, applicando modificatori alle regole di base.
...
All'interno di ogni fase, l'azione si artiola nei turni individuali dei partecipanti. Durante le fasi di socmmessa, le fiches convergono in un piatto centrale. Le carte vengono fornite da un mazzo,  che include sia le carte classiche che speciali.
...
L'architettura adottata per il progetto ChaosJack segue il pattern architteturale MVC(Model-View-Cotroller). Questo pattern consente di avere una chiara separazione delle responsablità e una maggiore flessibilità in fase di sviluppo, permettendoci di separare la logica di gioco dalla sua rappresentazione grafica.
...
\begin{itemize}
	...
	\item Table 
	\item ViewManager
	\item GameFlowController
	\item ActionController
\end{itemize}
Il GameEngine e Table si occupano di gestire e mantenere in modo consistente lo stato interno del gioco (il mazzo di carte, i portafogli dei partecipanti, il calcolo dei punteggi nelle mani). Inoltre il GameEngine valida ed esegue tutte le mutazioni dello stato di gioco secondo le regole tradizionali del BlackJack e l'applicazione dei relativi modificatori speciali. Il controller rappresenta il motore attivo che si occupa di dettare  i tempi 
e l'ordine delle diverse fasi della partita (regolando i turni tra i giocatori presenti nel tavolo).  
Il ViewManager ha il ruolo generale di gestire la visualizzazione dei componenti all'interno della finestra, mostrando l'evoluzione della partita. Si occupa, inoltre, di gestire la fase di navigazione del menù, consentendo di avviare una nuova partita o vedere le statistiche dei round giocati.
